...
int main() {
    ...
    std::sort(std::begin(v1), std::end(v1), [](std::string const& a, std::string const& b){
        return std::lexicographical_compare(a.begin() + a.find_last_of('/'), a.end(), b.begin() + b.find_last_of('/'), b.end());
    });
    std::sort(std::begin(v2), std::end(v2), [](std::string const& a, std::string const& b){
        return std::lexicographical_compare(a.begin() + a.find_last_of('/'), a.end(), b.begin() + b.find_last_of('/'), b.end());
    });
    ...
    std::transform(v1.begin(), v1.end(), v2.begin(), std::inserter(v1_to_v2_map, v1_to_v2_map.end()), [](string a, string b){
